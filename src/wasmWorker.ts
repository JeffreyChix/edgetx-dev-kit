...
/**
 * Node.js worker_threads WASM worker.
 * Adapted from packages/simulator-ui/src/lib/worker.ts.
 * Uses parentPort instead of globalThis.onmessage/postMessage.
 * FsProxyClient is inlined since it's a pure JS class that works identically
 * in Node.js worker threads (Atomics.wait is allowed in worker threads).
 */
...
// ---------------------------------------------------------------------------
// Inlined protocol constants (from fs-proxy-protocol.ts)
// ---------------------------------------------------------------------------
...
function deserializeStat(buf: Uint8Array, offset: number) {
...
// ---------------------------------------------------------------------------
// Inlined BigIntStatsProxy and DirentProxy
// ---------------------------------------------------------------------------
...
class DirentProxy {
  ...
  constructor(name: string, type: number) { this.name = name; this._type = type; }
...
class FsProxyClient {
  ...
  openSync(path: string, flags: number | string, mode = 0o666): number {
...
// ---------------------------------------------------------------------------
// Thread message handler (same logic as browser worker.ts)
// ---------------------------------------------------------------------------
...
const handler = new ThreadMessageHandler({
  ...
  async onLoad({ wasmModule, wasmMemory }) {
    ...
    const initialized = wasiThreads.initialize(instance, wasmModule, wasmMemory);
...
parentPort!.on("message", (data: any) => {
  if (data?.type === "analog-buffer") {
    analogValues = new Int16Array(data.buffer);
    return;
  }
  if (data?.type === "lcd-sync") {
    lcdSync = new Int32Array(data.buffer);
    return;
  }
  if (data?.type === "wake-buffer") {
    wakeBuffer = data.buffer;
    return;
  }
  if (data?.type === "fs-channel") {
    fsClient = new FsProxyClient(data.ctrlBuffer, data.dataBuffer, wakeBuffer ?? undefined);
    return;
  }
  // Wrap the data as a MessageEvent-like object for the handler
  handler.handle({ data } as MessageEvent);
});
